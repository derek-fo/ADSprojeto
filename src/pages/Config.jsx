...
export default function Config() {
  ...
  async function handleSalvarNome() {
    if (nome.trim() === '') return setMensagem('O nome do perfil não pode estar vazio!');
    try {
  ...
  async function handleSalvarEmail() {
    if (email.trim() === '') return setMensagem('O e-mail não pode estar vazio!');
    try {
  ...
  async function handleAlterarSenha() {
    if (senhaAtual === '' || novaSenha === '') return setMensagem('Preencha a senha atual e a nova senha!');
    try {
  ...
  return (
    <div className="min-h-screen bg-bg text-text-light p-4 md:p-8 pb-20">
      <div className="max-w-5xl mx-auto flex flex-col md:flex-row gap-6">
        ...
        <div className={`w-full md:w-1/4 bg-bg-card border border-line p-4 rounded-xl h-fit space-y-2 ${viewMobile === 'menu' ? 'block' : 'hidden'} md:block`}>
          <button
            onClick={() => nav('/inicio')}
          ...
          <h3 className="text-text-muted text-xs font-bold uppercase tracking-wide px-2 mb-4">Configurações</h3>
        ...
        <div className={`flex-1 bg-bg-card border border-line p-6 rounded-xl space-y-6 ${viewMobile === 'content' ? 'block' : 'hidden'} md:block`}>
          <button
          ...
          {tabAtiva === 'perfil' && (
            <>
              <div>
                <h2 className="text-white font-[Oswald] text-xl font-bold uppercase tracking-wide">Minha Conta</h2>
                <p className="text-xs text-text-muted mt-1">Gerencie suas informações de perfil, e-mail e credenciais de acesso.</p>
              </div>

              <div className="border-t border-line pt-4 space-y-2">
                <label className="text-text-muted text-xs block font-semibold uppercase">Nome do Perfil</label>
              ...
              <div className="border-t border-line pt-4 space-y-2">
                <label className="text-text-muted text-xs block font-semibold uppercase">Endereço de E-mail</label>
                <div className="flex flex-col sm:flex-row gap-2">
                  <input
                    type="email"
                    value={email}
              ...
              <div className="border-t border-line pt-4 space-y-2">
                <label className="text-text-muted text-xs block font-semibold uppercase">Trocar Senha</label>
              ...
              <div className="border-t border-line pt-6">
                <div className="bg-red-dark/10 border border-red-dark/40 p-4 rounded-md flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <div>
                    <h4 className="text-sm font-semibold text-red uppercase">Zona de Perigo</h4>
                    <p className="text-xs text-text-muted">Ao excluir sua conta, todos os seus treinos e históricos salvos serão apagados permanentemente.</p>
                  </div>
                  <button
          ...
          {tabAtiva === 'vincularcontas' && <Placeholder texto="Vinculação de Contas" />}
          {tabAtiva === 'notificacoes'   && <Placeholder texto="Notificações" />}
          {tabAtiva === 'feedback'       && <Placeholder texto="Feedback" />}
          {tabAtiva === 'ajudaesuporte'  && <Placeholder texto="Ajuda e Suporte" />}

          {tabAtiva === 'logout' && (
            <div className="border-t border-line pt-6">
              <div className="bg-red-dark/10 border border-red-dark/40 p-4 rounded-md flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                  <h4 className="text-sm font-semibold text-red uppercase">Quer sair da sua conta?</h4>
