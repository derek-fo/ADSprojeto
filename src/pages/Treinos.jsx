  ...
  return (
    <div className="min-h-screen bg-bg text-text-light p-4 md:p-8 pb-24">
      <div className="max-w-5xl mx-auto w-full space-y-6">
        ...
        <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-line pb-4 gap-4">
          <div>
            <h1 className="text-white font-[Oswald] text-2xl font-bold tracking-[3px] uppercase">Seus Treinos</h1>
            <p className="text-xs text-text-muted mt-0.5">Gerencie e selecione suas rotinas de exercícios salvas.</p>
          </div>
          <button
            onClick={() => nav('/novo-treino')}
            className="h-[42px] px-4 bg-red text-white font-[Oswald] text-xs font-bold tracking-[1px] rounded flex items-center justify-center gap-1.5 hover:opacity-85 transition-opacity uppercase self-start sm:self-auto"
          >
        ...
        {erro && (
          <p className="bg-red/10 border border-red text-red text-xs rounded px-3 py-2 text-center">{erro}</p>
        )}

        {carregando && <p className="text-text-muted text-sm text-center">Carregando sua biblioteca de treinos...</p>}

        {!carregando && listaTreinos.length === 0 && (
          <div className="bg-bg-card border border-line rounded-xl p-8 text-center space-y-3">
            <Dumbbell className="mx-auto text-text-muted opacity-40" size={40} />
            <p className="text-text-muted text-sm max-w-sm mx-auto">
              Nenhum treino cadastrado até o momento. Comece criando uma rotina personalizada de musculação ou funcional.
            </p>
          </div>
        )}
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-[11px] text-text-muted group-hover:text-white transition-colors">Ver Ficha →</span>
                  </div>
